...
//#endif
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		model = glm::scale(model, scale_vector + glm::vec3(-0.7, -0.7, -0.7));
		...
		model = glm::mat4(1);
		model = glm::translate(model, translate_vector + glm::vec3(-0.25, -0.25, 0.0));
		if (rotateX)
		{
			model = glm::rotate(model, angle, glm::vec3(1.0f, 0.0f, 0.0f));
		}
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mode)
{
	...
	{
		translate_vector += glm::vec3(0.0, 0.0, 0.1);
	}
	...
	{
		translate_vector += glm::vec3(-0.1, 0.0, 0.0);
	}
